...
class ExternalCodecJvmSuite extends munit.FunSuite:
  ...
  test("external metadata and executor capabilities fail independently at open"):
    ...
    assertEquals(
      ...
      Left("unsupported codec extension 'io.github.canardlapin.zarr4s.test.xor'")
    )
    assertEquals(
      SyncZarr
        .openArray(
        ...
        .left
        .map(_.message),
      Left("io.github.canardlapin.zarr4s.test.xor is unavailable on synchronous runtime")
    )
  ...
  test("external provider reads and writes a direct array without core branching"):
    ...
    val root = Files.createTempDirectory("zarr4s-external-codec-direct")
    ...
    assert(rendered.contains("\"name\":\"io.github.canardlapin.zarr4s.test.xor\""))
  ...
  test("external provider reads and writes an indexed shard"):
    ...
    val root = Files.createTempDirectory("zarr4s-external-codec-sharded")
    val target = root.resolve("array.zarr")
    val provider = new ChunkProvider:
      def chunk(
          coordinate: ChunkCoordinate,
