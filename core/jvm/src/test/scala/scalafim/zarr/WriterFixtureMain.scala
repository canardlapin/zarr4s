...
object WriterFixtureMain:
  ...
  private def writeUInt64(target: Path): Unit =
    ...
    val block = PrimitiveBlock.UInt64(OwnedLongs.copyOf(Array(
      0L,
      1L,
      Long.MaxValue,
      Long.MinValue,
  ...
  private def writeScalars(parent: Path): Unit =
    val values = Vector[(String, String, PrimitiveBlock)](
      ("bool", "false", PrimitiveBlock.Bool(OwnedBooleans.copyOf(
        Array(false, true, false, true, true, false)
      ))),
      ("int8", "0", PrimitiveBlock.Int8(OwnedBytes.copyOf(
        Array[Byte](Byte.MinValue, -1, 0, 1, 42, Byte.MaxValue)
      ))),
      ("uint8", "0", PrimitiveBlock.UInt8(OwnedBytes.copyOf(
        Array[Byte](0, 1, 127, -128, -2, -1)
      ))),
      ("int16", "0", PrimitiveBlock.Int16(OwnedShorts.copyOf(
        Array[Short](Short.MinValue, -1, 0, 1, 42, Short.MaxValue)
      ))),
      ("uint16", "0", PrimitiveBlock.UInt16(OwnedShorts.copyOf(
        Array[Short](0, 1, 32767, -32768, -2, -1)
      ))),
      ("int32", "0", PrimitiveBlock.Int32(OwnedInts.copyOf(
        Array(Int.MinValue, -1, 0, 1, 42, Int.MaxValue)
      ))),
      ("uint32", "0", PrimitiveBlock.UInt32(OwnedInts.copyOf(
        Array(0, 1, Int.MaxValue, Int.MinValue, -2, -1)
      ))),
      ("int64", "0", PrimitiveBlock.Int64(OwnedLongs.copyOf(
        Array(Long.MinValue, -1L, 0L, 1L, 42L, Long.MaxValue)
      ))),
      ("uint64", "0", PrimitiveBlock.UInt64(OwnedLongs.copyOf(
        Array(0L, 1L, Long.MaxValue, Long.MinValue, -2L, -1L)
      ))),
      ("float32", "0.0", PrimitiveBlock.Float32(OwnedFloats.copyOf(
        Array(0.0f, -0.0f, 1.5f, -2.25f, Float.PositiveInfinity, Float.NegativeInfinity)
      ))),
      ("float64", "0.0", PrimitiveBlock.Float64(OwnedDoubles.copyOf(
        Array(0.0, -0.0, 1.5, -2.25, Double.PositiveInfinity, Double.NegativeInfinity)
      )))
    )
    values.foreach: (name, fill, block) =>
      val configuration = if name == "bool" || name.endsWith("8") then ""
  ...
  private def linearProvider(grid: RegularGrid, fillValue: Short): ChunkProvider = new ChunkProvider:
    def chunk(coordinate: ChunkCoordinate, storedShape: Shape): Either[ZarrError, ChunkPayload] =
      val count = storedShape.elementCount.fold(error => throw new IllegalStateException(error.message), identity)
