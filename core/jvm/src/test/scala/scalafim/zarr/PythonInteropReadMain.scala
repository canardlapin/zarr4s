...
object PythonInteropReadMain:
  ...
  private def assertCommonScalars(parent: Path): Unit =
    read(parent.resolve("python-bool.zarr")) match
      case PrimitiveBlock.Bool(values) => require(values.toArray.toVector == Vector(false, true, false, true, true, false))
      case found => throw IllegalStateException(s"expected bool, found $found")
    read(parent.resolve("python-int8.zarr")) match
      case PrimitiveBlock.Int8(values) => require(values.toArray.toVector == Vector[Byte](-128, -1, 0, 1, 42, 127))
      case found => throw IllegalStateException(s"expected int8, found $found")
    read(parent.resolve("python-uint8.zarr")) match
      case PrimitiveBlock.UInt8(values) => require(values.toArray.toVector == Vector[Byte](0, 1, 127, -128, -2, -1))
      case found => throw IllegalStateException(s"expected uint8, found $found")
    read(parent.resolve("python-int16.zarr")) match
      case PrimitiveBlock.Int16(values) => require(values.toArray.toVector == Vector[Short](Short.MinValue, -1, 0, 1, 42, Short.MaxValue))
      case found => throw IllegalStateException(s"expected int16, found $found")
    read(parent.resolve("python-uint16.zarr")) match
      case PrimitiveBlock.UInt16(values) => require(values.toArray.toVector == Vector[Short](0, 1, 32767, -32768, -2, -1))
      case found => throw IllegalStateException(s"expected uint16, found $found")
    assertInt32(parent.resolve("python-int32.zarr"), Vector(Int.MinValue, -1, 0, 1, 42, Int.MaxValue))
    read(parent.resolve("python-uint32.zarr")) match
      case PrimitiveBlock.UInt32(values) => require(values.toArray.toVector == Vector(0, 1, Int.MaxValue, Int.MinValue, -2, -1))
      case found => throw IllegalStateException(s"expected uint32, found $found")
    read(parent.resolve("python-int64.zarr")) match
      case PrimitiveBlock.Int64(values) => require(values.toArray.toVector == Vector(Long.MinValue, -1L, 0L, 1L, 42L, Long.MaxValue))
      case found => throw IllegalStateException(s"expected int64, found $found")
    read(parent.resolve("python-uint64.zarr")) match
      case PrimitiveBlock.UInt64(values) => require(values.toArray.toVector == Vector(0L, 1L, Long.MaxValue, Long.MinValue, -2L, -1L))
    ...
    read(parent.resolve("python-float32.zarr")) match
      case PrimitiveBlock.Float32(values) =>
        val expected = Array(0.0f, -0.0f, 1.5f, -2.25f, Float.PositiveInfinity, Float.NegativeInfinity)
        require(values.toArray.map(java.lang.Float.floatToRawIntBits).sameElements(
    ...
    read(parent.resolve("python-float64.zarr")) match
      case PrimitiveBlock.Float64(values) =>
        val expected = Array(0.0, -0.0, 1.5, -2.25, Double.PositiveInfinity, Double.NegativeInfinity)
        require(values.toArray.map(java.lang.Double.doubleToRawLongBits).sameElements(
  ...
  private def assertInt32(path: Path, expected: Vector[Int]): Unit = read(path) match
    case PrimitiveBlock.Int32(values) => require(values.toArray.toVector == expected, s"$path values differ")
  ...
  private def assertHierarchy(path: Path, expectedFormat: ZarrFormat): Unit =
    val store = JvmFileStore.open(path)
      .fold(detail => throw IllegalArgumentException(detail), identity)
    val root = SyncZarr.openGroup(
      store,
    ...
    require(
      root.children.fold(error => throw IllegalStateException(error.message), identity)
        .map(_.path.value) == Vector("bold", "derived"),
      s"$path root children differ"
    )
    val bold = root.openArray("bold")
      .fold(error => throw IllegalStateException(error.message), identity)
    ...
    val derived = root.openGroup("derived")
      .fold(error => throw IllegalStateException(error.message), identity)
    require(
      derived.children.fold(error => throw IllegalStateException(error.message), identity)
        .map(_.path.value) == Vector("derived/mask"),
      s"$path derived children differ"
    )
    val mask = root.openArray("derived/mask")
      .fold(error => throw IllegalStateException(error.message), identity)
    readOpened(mask) match
      case PrimitiveBlock.UInt8(found) =>
        require(found.toArray.toVector == Vector[Byte](1, 1, 0, 0, 1, 1), s"$path mask values differ")
      case found => throw IllegalStateException(s"expected uint8, found $found")

  private def assertFactored(parent: Path): Unit =
    val path = parent.resolve("python-factored.zarr")
    val expected = JsonParser.parse(Files.readString(parent.resolve("python-factored-expected.json"))) match
      case Right(JsonValue.Arr(values)) => values.map:
        case JsonValue.Num(number) => number.toLongExact match
          case Right(found) if found >= Int.MinValue.toLong && found <= Int.MaxValue.toLong => found.toInt
          case Right(found) => throw IllegalStateException(s"factored expected value $found is outside Int")
    ...
    val store = JvmFileStore.open(path)
      .fold(detail => throw IllegalArgumentException(detail), identity)
    val opened = SyncZarr.openArray(store, runtime = JvmCodecRuntime.portable)
      .fold(error => throw IllegalStateException(error.message), identity)
    val selection = FactoredSelection.within(opened.descriptor.shape, Vector(
      AxisSelector.Indices(AxisIndices.from(Vector(6L, 1L, 6L, 0L))
        .fold(error => throw IllegalArgumentException(error.message), identity)),
      AxisSelector.Slice(AxisSlice(1L, 8L, 3L)
        .fold(error => throw IllegalArgumentException(error.message), identity)),
      AxisSelector.Indices(AxisIndices.from(Vector(8L, 2L, 8L, 0L))
        .fold(error => throw IllegalArgumentException(error.message), identity))
    )).fold(error => throw IllegalArgumentException(error.message), identity)
    val materialized = opened.read(selection)
      .fold(error => throw IllegalStateException(error.message), identity)
      ...
      (output, fragment) =>
        ...
        while element < values.length do
          ...
          while axis < cursor.length do
            val placement = fragment.placement.axis(axis)
              .fold(error => throw IllegalStateException(error.message), identity)
            val outputIndex = placement.outputIndex(cursor(axis))
              .fold(error => throw IllegalStateException(error.message), identity)
  ...
  private def readOpened(opened: OpenedArray): PrimitiveBlock =
    val origin = Coordinate.from(Vector.fill(opened.descriptor.shape.rank.toInt)(0L))
      .fold(error => throw IllegalArgumentException(error.message), identity)
    val region = Region.within(opened.descriptor.shape, origin, opened.descriptor.shape)
      .fold(error => throw IllegalArgumentException(error.message), identity)
    val result = opened.readRegion(region)
      .fold(error => throw IllegalStateException(error.message), identity)
    result.block
