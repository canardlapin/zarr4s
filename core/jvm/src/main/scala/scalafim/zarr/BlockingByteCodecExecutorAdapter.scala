...
/** Explicit bridge from a synchronous byte codec to the portable async runtime.
  *
  * The captured execution context must be dedicated to blocking codec work.
  * The reader's callback context is deliberately not used for the algorithm.
  */
  ...
  def apply(
    ...
    new AsyncByteCodecExecutor:
      ...
      def decode(
        ...
        Future(executor.decode(codec, encoded, expectedDecoded, limits))(
