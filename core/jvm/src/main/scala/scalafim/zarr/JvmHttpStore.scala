...
final class JvmHttpStore private (
  ...
  def read(key: StoreKey, range: ByteRange): Either[StoreError, OwnedBytes] =
    if range.length.toLong == 0L then Right(OwnedBytes.empty)
      ...
      val request = HttpRequest.newBuilder(uri(key))
  ...
  private def send(
    ...
    catch case NonFatal(error) => Left(StoreError.Transport(key, error.getMessage, transient = true))
...
object JvmHttpStore:
  def apply(baseUri: URI, client: HttpClient = HttpClient.newHttpClient()): Either[String, JvmHttpStore] =
