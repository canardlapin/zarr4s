...
/** Metadata emitted by Zarr-Python 3.2.1.
  *
  * Each array was created with the bytes serializer, no compressors, and Zarr format 3. Keeping
  * these as source literals makes the same independent fixtures available to MUnit on the JVM and
  * Scala.js.
  */
