...
class FragmentSuite extends munit.FunSuite:
  ...
  private def selection(shape: Shape): FactoredSelection = zvalue(
  ...
  test("direct fragment fold emits compact decoded and fill pieces with exact placement"):
    ...
    store.clearTrace()
    val folded = zvalue(
      opened.foldFragments(selected, Vector.empty[ChunkFragment])((fragments, fragment) =>
        Right(FragmentControl.Continue(fragments :+ fragment))
      )
    )
    assertEquals(
  ...
  test("fragment fold propagates consumer and decode errors without further reads"):
    ...
    var calls = 0
    val decoded = corruptOpened.foldFragments(selection(corruptOpened.descriptor.shape), ()):
      (_, _) =>
        calls += 1
  ...
  test("fragment execution is runtime-rank for scalar and rank-five arrays"):
    ...
    val scalarFold = zvalue(
      scalar.foldFragments(
    ...
    val rankFiveFold = zvalue(
      rankFive.foldFragments(
        selected,
        Vector.empty[ChunkFragment]
      )((fragments, fragment) => Right(FragmentControl.Continue(fragments :+ fragment)))
    )
    ...
    rankFiveFold.state.head.values match
      case PrimitiveBlock.Int16(found) =>
        assertEquals(found.toArray.toVector, Vector[Short](8, 7, 8))
