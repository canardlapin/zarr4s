...
/** Test-only codec provider: it deliberately lives outside `zarr4s`. */
...
object ExternalXorCodec:
  val name = "io.github.canardlapin.zarr4s.test.xor"
...
object ExternalXorCapability extends CodecCapability:
  ...
  def compile(
    ...
    case Some(JsonValue.Num(number)) =>
      number.toLongExact.flatMap: mask =>
...
object ExternalXorFixture:
  ...
  val syncRuntime: SyncCodecRuntime = value(
    SyncCodecRuntime(
      "external test runtime",
      Vector(ExternalXorSyncExecutor)
    )
  )
  val asyncRuntime: AsyncCodecRuntime = value(
    AsyncCodecRuntime(
