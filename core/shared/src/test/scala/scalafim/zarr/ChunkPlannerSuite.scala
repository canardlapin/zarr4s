...
class ChunkPlannerSuite extends munit.FunSuite:
  ...
  test("region planner covers a request exactly in deterministic C order"):
    ...
    assertEquals(plan.demands.map(_.coordinate.toVector), Vector(
      Vector(0L, 0L), Vector(0L, 1L), Vector(0L, 2L),
      Vector(1L, 0L), Vector(1L, 1L), Vector(1L, 2L)
    ))
  ...
  test("empty region produces no store reads"):
    ...
    assertEquals(value(ChunkPlanner.planRegion(grid, region)), ReadPlan(Vector.empty, ReadPlanStats(0, 0L)))
  ...
  test("point planner groups chunks while preserving result order and duplicates"):
    ...
    val grid = value(RegularGrid(shape, value(Shape(4L, 4L))))
    val points = value(CoordinateBatch.within(shape, Seq(
    ...
    assertEquals(plan.demands.map(_.coordinate.toVector), Vector(
      Vector(0L, 0L), Vector(0L, 1L), Vector(1L, 0L)
    ))
  ...
  test("rank-five planning is a normal kernel operation"):
    ...
    val region = value(Region.within(
      shape,
