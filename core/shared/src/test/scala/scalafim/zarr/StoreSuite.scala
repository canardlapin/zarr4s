...
class StoreSuite extends munit.FunSuite:
  ...
  test("memory store has exact range, whole, length, and trace semantics"):
    ...
    assertEquals(store.read(key("a/b"), range(1L, 3L)).map(_.toArray.toVector), Right(Vector[Byte](1, 2, 3)))
  ...
  test("range coalescing is deterministic and bounded"):
    ...
    val result = zvalue(RangeCoalescer.coalesce(
      ranges,
      CoalescingLimits(3L, zvalue(ByteCount(40L)))
    ))
    assertEquals(result.map(found => found.range.offset -> found.range.length.toLong), Vector(
      0L -> 17L,
      30L -> 5L,
      100L -> 10L
    ))
    assertEquals(result.head.members.map(member => member.value -> member.relativeOffset), Vector(
      "a" -> 0,
      "b" -> 12
    ))
