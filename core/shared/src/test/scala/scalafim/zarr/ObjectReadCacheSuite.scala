...
class ObjectReadCacheSuite extends munit.FunSuite:
  ...
  test("async identical reads collapse to one request and return independent bytes"):
    ...
    Future.sequence(Vector(first, second)).flatMap: results =>
      ...
      reader.readAll(objectKey, count(8L)).map: third =>
  ...
  test("async identical ranges and lengths each collapse to one request"):
    ...
    Future.sequence(ranges).zip(Future.sequence(lengths)).map: (rangeResults, lengthResults) =>
  ...
  test("async store errors and failed futures are never retained"):
    ...
    val storeErrors = new AsyncObjectReader:
      ...
      def readAll(
        ...
        errors += 1
        if errors == 1 then Future.successful(Left(StoreError.Transport(key, "once", transient = true)))
    ...
    errorReader.readAll(objectKey, count(8L)).flatMap: first =>
      assert(first.isLeft)
      errorReader.readAll(objectKey, count(8L)).flatMap: second =>
        ...
        failedReader.readAll(objectKey, count(8L)).transformWith:
          case scala.util.Failure(_) => failedReader.readAll(objectKey, count(8L)).map: retried =>
  ...
  test("portable AsyncZarr re-reads direct regions, factored selections, and shards from cache"):
    ...
    def directStore(): AsyncMemoryStore = zvalue(AsyncMemoryStore(Map(
    ...
    def repeatedDirectRegion(): Future[Unit] =
      ...
      AsyncZarr.openArray(reader).flatMap:
        case Left(error) => fail(error.message)
        case Right(opened) =>
