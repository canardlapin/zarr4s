...
/** Identity of the immutable store revision represented by one read cache.
  *
  * Callers must choose a new namespace whenever object contents may have changed. The cache
  * deliberately has no implicit or process-global identity.
  */
opaque type CacheNamespace = String

object CacheNamespace:
  def from(value: String): Either[ZarrError, CacheNamespace] =
    if value.isEmpty then Left(ZarrError.InvalidSelection("cache namespace must not be empty"))
  ...
  extension (namespace: CacheNamespace) inline def value: String = namespace
...
/** A bounded deterministic least-recently-used cache for one immutable revision.
  *
  * Entries are exact whole objects, exact ranges, or object lengths. Range requests may be
  * satisfied from a cached containing range or whole object. Every insertion and return crosses an
  * ownership boundary by copying bytes.
  */
final class ObjectReadCache private (
  ...
  private[zarr4s] def cachedRange(
      key: StoreKey,
      requested: ByteRange
    ...
    entries.get(exact) match
      ...
      case _ =>
        ...
        while iterator.hasNext do
          ...
          entry.value match
            case CacheEntryValue.Bytes(bytes) =>
              candidateKey match
                case CacheEntryKey.Whole(candidate)
                    if candidate == key &&
                      requestedEnd <= bytes.byteCount.toLong =>
                  chosen = chooseContaining(chosen, candidateKey, 0L, bytes)
                case CacheEntryKey.Range(candidate, offset, length)
                    if candidate == key &&
  ...
  private[zarr4s] def cachedWhole(key: StoreKey): Option[OwnedBytes] = synchronized:
  ...
  private[zarr4s] def cachedLength(key: StoreKey): Option[Long] = synchronized:
  ...
  private[zarr4s] def putWhole(key: StoreKey, bytes: OwnedBytes): Unit = synchronized:
    insert(
      CacheEntryKey.Whole(key),
      CacheEntry(CacheEntryValue.Bytes(OwnedBytes.copyOf(bytes.values)), bytes.byteCount.toLong)
    )

  private[zarr4s] def putLength(key: StoreKey, length: Long): Unit = synchronized:
    if length >= 0L then
      insert(CacheEntryKey.Length(key), CacheEntry(CacheEntryValue.Length(length), 0L))

  private[zarr4s] def hit(servedBytes: Long): Unit = synchronized:
    hitsValue = addSaturated(hitsValue, 1L)
    servedBytesValue = addSaturated(servedBytesValue, servedBytes)

  private[zarr4s] def miss(): Unit = synchronized:
    missesValue = addSaturated(missesValue, 1L)

  private[zarr4s] def downstream(): Unit = synchronized:
    downstreamRequestsValue = addSaturated(downstreamRequestsValue, 1L)

  private[zarr4s] def fetched(bytes: Long): Unit = synchronized:
    fetchedBytesValue = addSaturated(fetchedBytesValue, bytes)

  private[zarr4s] def served(bytes: Long): Unit = synchronized:
    servedBytesValue = addSaturated(servedBytesValue, bytes)

  private[zarr4s] def joined(): Unit = synchronized:
