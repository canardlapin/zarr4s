...
final class IntOffsets private (private[zarr4s] val values: Array[Int]):
...
object IntOffsets:
  private[zarr4s] def unsafe(values: Array[Int]): IntOffsets =
...
enum ChunkCopy:
  ...
  private[zarr4s] case FactoredCopy(copy: FactoredChunkCopy)
      ...
      region.extent.elementCount.flatMap: requested =>
        if region.isEmpty then Right(ReadPlan(Vector.empty, ReadPlanStats(0, requested)))
          ...
          while axis < rank do
            ...
            last(axis) = (region.origin.values(axis) + region.extent.values(axis) - 1L) /
            ...
            if count > limits.maxChunks.toLong then
              ...
              enumerate(first, last): coordinateValues =>
                ...
                while innerAxis < rank do
                  ...
                  val chunkEnd = math.min(
                    origin + grid.chunkShape.values(innerAxis),
                    grid.arrayShape.values(innerAxis)
                  )
  ...
  private def planAxis(
    ...
    case BoundAxisSelection.Strided(start, step, length) =>
      ...
      while outputStart < length do
        val scaled =
          LongArrays.checkedMultiply(outputStart, step, "factored slice coordinate") match
            case Left(error)  => return Left(error)
            case Right(found) => found
    ...
    case BoundAxisSelection.Gather(indices) =>
      if indices.length > limits.maxAxisIndexEntries then
        ...
        val grouped =
          mutable.HashMap.empty[Long, (mutable.ArrayBuffer[Long], mutable.ArrayBuffer[Long])]
        var output = 0
        ...
        if grouped.size > limits.maxChunks then
          ...
          Right(grouped.keys.toVector.sorted.map: chunk =>
