...
private[zarr4s] object ChunkGeometry:
  /** The decoded shape of every regular-grid chunk, including border chunks.
    *
    * Zarr v3 border chunks retain the grid's nominal chunk shape. Elements outside the logical
    * array extent are represented by the array fill value.
    */
...
private[zarr4s] sealed trait PrimitiveBlockBuilder:
  def size: Int
  def copyElement(
...
private[zarr4s] object PrimitiveBlockBuilder:
  def apply(
  ...
  private def copyRegion(
    ...
    if destinationShape.rank.toInt != rank || copy.extent.rank.toInt != rank then
      ...
      copy.extent.elementCount match
