...
final class FragmentAxis private[zarr4s] (private val run: IndexRun):
  ...
  private[zarr4s] def unsafeOutputIndex(position: Long): Long = run(position)

final class FragmentPlacement private[zarr4s] (
  ...
  private[zarr4s] def unsafeOutputIndex(axis: Int, position: Long): Long =
    axes(axis).unsafeOutputIndex(position)

private[zarr4s] object FragmentPlacement:
...
final class ChunkFragment private[zarr4s] (
...
private[zarr4s] object ChunkFragment:
  def decoded(
    ...
    PrimitiveBlockBuilder
      .compact(dataType, fill, decoded, decodedShape, copy)
      .map: compacted =>
        new ChunkFragment(
          coordinate,
          compacted,
...
final class FragmentReceipt private[zarr4s] (
  ...
  require(
    visitedChunks >= 0 && visitedChunks <= plannedChunks,
    "visitedChunks must be within the plan"
  )
  require(decodedChunks >= 0, "decodedChunks must be non-negative")
  require(fillChunks >= 0, "fillChunks must be non-negative")
  require(
    decodedChunks + fillChunks == visitedChunks,
    "decoded and fill chunks must account for visits"
  )
  require(emittedFragments == visitedChunks, "one fragment must be emitted per visited chunk")
  require(
    emittedElements >= 0L && emittedElements <= requestedElements,
    "emitted elements must be within request"
  )
  require(elementByteWidth > 0, "elementByteWidth must be positive")
  require(plannedShards >= 0, "plannedShards must be non-negative")
  require(
    !completed || visitedChunks == plannedChunks,
    "completed receipt must visit every planned chunk"
  )
  require(
    !completed || emittedElements == requestedElements,
    "completed receipt must emit every requested element"
  )
...
private[zarr4s] final class FragmentMetrics private (
    val plannedChunks: Int,
    val requestedElements: Long,
    val elementByteWidth: Int,
    val plannedShards: Int
...
private[zarr4s] object FragmentMetrics:
  def checked(
      plannedChunks: Int,
