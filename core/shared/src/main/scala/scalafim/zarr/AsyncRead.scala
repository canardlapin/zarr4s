...
private[zarr] object AsyncBatch:
  def traverseBounded[A, B](
    ...
    if values.isEmpty then Future.successful(Vector.empty)
      ...
      Future.sequence(workers).map: _ =>
        Vector.tabulate(values.length)(index => results(index).asInstanceOf[B])

final class AsyncOpenedArray private[zarr] (
  ...
  def read(
    ...
    if selection.arrayShape != descriptor.shape then
      Future.successful(Left(ZarrError.InvalidSelection(
      ...
      PrimitiveBlockBuilder(
        ...
        case Right(builder) =>
          val folded = foldFragments(selection, builder, limits): (current, fragment) =>
            Future.successful(PrimitiveBlockBuilder.applyFragment(
              current,
              fragment,
              selection.outputShape
  ...
  def foldFragments[S](
    ...
    case PhysicalLayout.Sharded(sharded, innerCodecs, _, location, outerCodecs) =>
      if outerCodecs.nonEmpty then Future.successful(Left(ZarrError.UnsupportedRead(
        "outer codecs after sharding_indexed prevent bounded partial reads"
      )))
      else foldShardedFragments(
        selection,
        initial,
        sharded,
        innerCodecs,
  ...
  private def foldDirectFragments[S](
    ...
    ChunkPlanner.planFactored(descriptor.grid, selection, limits.planning) match
      ...
      case Right(plan) if plan.demands.length > limits.maxObjects =>
        Future.successful(Left(ZarrError.ResourceLimit(
          "read objects",
          limits.maxObjects,
          plan.demands.length
        )))
      case Right(plan) =>
        ...
        def loop(index: Int, state: S): Future[Either[ZarrError, FragmentFoldResult[S]]] =
          if index >= plan.demands.length then
            ...
            demand.copy match
              case ChunkCopy.FactoredCopy(copy) => chunkStoreKey(demand.coordinate) match
                case Left(error) => Future.successful(Left(error))
                case Right(key) =>
                  metrics.objectRequests += 1
                  store.readAll(key, limits.maxEncodedObjectBytes).flatMap:
                    case Left(StoreError.NotFound(_)) => deliver(
                      state,
                      ChunkFragment.fill(
                        demand.coordinate,
                        copy,
                    ...
                    case Right(encoded) =>
                      ...
                      ChunkGeometry.storedShape(descriptor.grid, demand.coordinate) match
                        case Left(error) => Future.successful(Left(error))
                        case Right(chunkShape) => runtime.decode(
                          encoded,
                          codecs,
                          descriptor.dataType,
                          chunkShape,
              ...
              case _ => Future.successful(Left(ZarrError.InvalidSelection(
  ...
  private def continueDirect[S](
    ...
    case Left(error) => Future.successful(Left(error))
    case Right(found) if found.stopped => Future.successful(Right(FragmentFoldResult(
      found.state,
        ...
        def loopShards(
          ...
          if index >= grouped.shards.length then
            ...
            chunkStoreKey(shard.coordinate) match
              case Left(error) => Future.successful(Left(error))
              case Right(shardKey) => fetchShardIndex(
                shard,
                sharded,
                location,
                limits,
                ...
                case Left(error) => Future.successful(Left(error))
                case Right(decodedIndex) => processShardFragments(
                  shard,
                  shardKey,
                  decodedIndex,
                  selection,
  ...
  private def fetchShardIndex(
      ...
      ShardIndexReadPlan(
        ...
        case Right(plan) =>
          ...
          checkedRangeRequest(metrics, limits) match
            case Left(error) => Future.successful(Left(error))
            case Right(_) => path.key(read.key.value) match
              case Left(error) => Future.successful(Left(error))
              case Right(key) =>
                metrics.objectRequests += 1
                metrics.rangeRequests += 1
                store.read(key, read.range).map:
    ...
    else chunkStoreKey(shard.coordinate) match
      case Left(error) => Future.successful(Left(error))
      case Right(key) =>
        metrics.objectRequests += 1
        metrics.lengthRequests += 1
        store.length(key).flatMap:
  ...
  private def processShardFragments[S](
    ...
    def loop(index: Int, state: S, missingShard: Boolean): Future[Either[ZarrError, FragmentProgress[S]]] =
      if index >= shard.innerChunks.length then
        ...
        inner.copy match
          case ChunkCopy.FactoredCopy(copy) =>
            ...
            if missingShard || decodedIndex.isEmpty then fill(nextMissing = true)
            else decodedIndex.get.entry(inner.localCoordinate) match
              case Left(error) => Future.successful(Left(error))
              ...
              case Right(ShardIndexEntry.Present(offset, length)) =>
                ...
                else checkedRangeRequest(metrics, limits) match
                  case Left(error) => Future.successful(Left(error))
                  case Right(_) => ByteRange(offset, length.toLong) match
                    case Left(error) => Future.successful(Left(error))
                    case Right(range) =>
                      metrics.objectRequests += 1
                      metrics.rangeRequests += 1
                      store.read(shardKey, range).flatMap:
                          ...
                          ChunkGeometry.storedShape(sharded.globalInnerGrid, global) match
                            ...
                            case Right(chunkShape) => runtime.decode(
                              ...
                              case Right(decoded) => deliver(
                                ...
                                case Left(error) => Future.successful(Left(error))
                                case Right(found) if found.stopped => Future.successful(Right(found))
                                case Right(found) => loop(index + 1, found.state, missingShard = false)
          case _ => Future.successful(Left(ZarrError.InvalidSelection(
            "fragment plan contains a non-factored copy"
          )))
  ...
  private def read(
    ...
    PrimitiveBlockBuilder(descriptor.dataType, descriptor.fillValue, outputShape) match
      case Left(error) => Future.successful(Left(error))
      case Right(builder) => descriptor.layout match
        case PhysicalLayout.Direct(codecs) =>
  ...
  private def readDirect(
    ...
    case Right(plan) =>
      AsyncBatch.traverseBounded(plan.demands, limits.maxConcurrentRequests): demand =>
        chunkStoreKey(demand.coordinate) match
          case Left(error) => Future.successful(Left(error))
          case Right(key) => store.readAll(key, limits.maxEncodedObjectBytes).map:
      ...
      .flatMap: fetched =>
        firstError(fetched) match
          ...
          case None =>
            ...
            val present = values.collect { case DirectFetch(demand, Some(bytes)) => demand -> bytes }
            AsyncBatch.traverseBounded(present, limits.maxConcurrentRequests): (demand, encoded) =>
              ChunkGeometry.storedShape(descriptor.grid, demand.coordinate) match
                case Left(error) => Future.successful(Left(error))
                case Right(chunkShape) => runtime.decode(
                    encoded,
      ...
      case Right((innerPlan, grouped)) =>
        fetchLengths(grouped, location, limits).flatMap:
          ...
          case Right(lengths) =>
            ...
            val present = grouped.shards.filterNot(shard => missing.contains(shard.coordinate.toVector))
            val presentPlan = ShardReadPlan(present, present.map(_.innerChunks.length).sum, present.length)
            ShardIndexReadPlan(
              presentPlan,
              sharded.innerChunksPerShard,
              location,
              ...
              case Left(error) => Future.successful(Left(error))
              case Right(indexPlan) => fetchIndexes(indexPlan, limits).flatMap:
                case Left(error) => Future.successful(Left(error))
                    ...
                    fetched.bytes match
                      ...
                      case Some(bytes) if decodeError.isEmpty =>
                        ...
                        ShardIndexCodec.decode(bytes, sharded.innerChunksPerShard, limits.shardIndex) match
                          ...
                          case Right(index) => indexes.update(fetched.read.shardCoordinate.toVector, index)
                      case Some(_) => ()
                  decodeError match
                    case Some(error) => Future.successful(Left(error))
                    case None =>
                          ...
                          ShardDataPlan.resolve(
                            ...
                            case Right(dataPlan) =>
                              fetchAndDecodeData(
                                ...
                                case Left(error) => Left(error)
                                case Right((dataRequests, dataBytes)) => Right(ReadResult(
                                  builder.result(),
                                  outputShape,
                                  ExecutionReceipt(
                                    (if location == IndexLocation.End then lengths.length else 0) +
  ...
  private def fetchLengths(
    ...
    if location == IndexLocation.Start then
      ...
      val fetched = AsyncBatch.traverseBounded(grouped.shards, limits.maxConcurrentRequests): shard =>
        chunkStoreKey(shard.coordinate) match
          case Left(error) => Future.successful(Left(error))
          case Right(key) => store.length(key).map:
  ...
  private def fetchIndexes(
    ...
    val fetched = AsyncBatch.traverseBounded(plan.reads, limits.maxConcurrentRequests): read =>
      path.key(read.key.value) match
        case Left(error) => Future.successful(Left(error))
        case Right(key) => store.read(key, read.range).map:
  ...
  private def fetchAndDecodeData(
    ...
    AsyncBatch.traverseBounded(taskValues, limits.maxConcurrentRequests): (shard, read) =>
    ...
    .flatMap: fetched =>
      firstError(fetched) match
        ...
        case None =>
          ...
          AsyncBatch.traverseBounded(memberValues, limits.maxConcurrentRequests): (shard, chunk, encoded) =>
            val global = globalInnerCoordinate(shard.coordinate, chunk.localCoordinate, sharded)
            ChunkGeometry.storedShape(sharded.globalInnerGrid, global) match
              case Left(error) => Future.successful(Left(error))
              case Right(chunkShape) => runtime.decode(
                  encoded,
...
final class AsyncOpenedGroup private[zarr] (
    store: AsyncObjectReader,
    val path: ZarrPath,
  ...
  def children: Either[ZarrError, Vector[HierarchyEntry]] = index match
    ...
    case None => Left(ZarrError.UnsupportedRead(
  ...
  def open(relativePath: String): Future[Either[ZarrError, AsyncOpenedNode]] =
    path.resolve(relativePath) match
      case Left(error) => Future.successful(Left(error))
      case Right(resolved) => index.flatMap(_.document(resolved)) match
        case Some(document) => Future.successful(AsyncZarr.openDocument(
          store,
          resolved,
          document,
          index,
        ...
        case None if consolidation == ConsolidationMode.Require =>
          Future.successful(Left(ZarrError.InvalidMetadata(
            "$.consolidated_metadata",
            s"required metadata does not contain '${resolved.value}'"
          )))
        case None => AsyncZarr.openNode(
          store,
          resolved,
          capabilities,
          limits,
...
object AsyncZarr:
  ...
  def openNode(
    ...
    readOptional(store, path, "zarr.json", limits.maxMetadataBytes).flatMap:
      ...
      case Right(Some(json)) => ZarrMetadata.parse(json) match
        ...
        case Right(ZarrNodeMetadata.Array(array)) => Future.successful(openDocument(
          store,
          path,
  ...
  private[zarr] def openDocument(
    ...
    case NodeKind.Group => document.groupMetadata.map: group =>
      AsyncOpenedNode.Group(new AsyncOpenedGroup(
        store,
        path,
        group,
    ...
    case NodeKind.Array => document.arrayDescriptor(capabilities).flatMap: descriptor =>
      OpenValidation.descriptor(descriptor, limits).flatMap: _ =>
        OpenValidation.codecPrograms(descriptor, runtime.validate).map: _ =>
          AsyncOpenedNode.Array(new AsyncOpenedArray(
            store,
            path,
            descriptor,
            document.format,
  ...
  private def openV2(
    ...
    val indexed = consolidation match
      ...
      case _ => readOptional(store, ZarrPath.root, ".zmetadata", limits.maxMetadataBytes).map:
        case Left(error) => Left(error)
        case Right(Some(json)) => HierarchyIndex.v2(ZarrPath.root, json, limits.hierarchy).map(Some.apply)
        case Right(None) if consolidation == ConsolidationMode.Require => Left(ZarrError.InvalidMetadata(
          "$.zmetadata",
          "required consolidated metadata is absent"
        ))
    ...
    indexed.flatMap:
      case Left(error) => Future.successful(Left(error))
      case Right(found) => found.flatMap(_.document(path)) match
        case Some(document) => Future.successful(openDocument(
          store,
          path,
          document,
          found,
  ...
  private def openV2Individual(
    ...
    readOptional(store, path, ".zarray", limits.maxMetadataBytes).flatMap:
      case Left(error) => Future.successful(Left(error))
      case Right(Some(arrayJson)) => readOptional(store, path, ".zattrs", limits.maxMetadataBytes).map:
        case Left(error) => Left(error)
        case Right(attributes) => V2Metadata.parseArray(arrayJson, attributes).flatMap: array =>
          openDocument(
            store,
            path,
      ...
      case Right(None) => readOptional(store, path, ".zgroup", limits.maxMetadataBytes).flatMap:
        case Left(error) => Future.successful(Left(error))
        case Right(Some(groupJson)) => readOptional(store, path, ".zattrs", limits.maxMetadataBytes).map:
          case Left(error) => Left(error)
          case Right(attributes) => V2Metadata.parseGroup(groupJson, attributes).map: group =>
            AsyncOpenedNode.Group(new AsyncOpenedGroup(
              store,
              path,
              group,
        ...
        case Right(None) => Future.successful(path.key(".zarray").flatMap: key =>
