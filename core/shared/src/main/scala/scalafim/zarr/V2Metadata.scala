...
object V2Metadata:
  ...
  private[zarr] def parseGroupObject(
      root: JsonObject,
      attributes: JsonObject
  ): Either[ZarrError, GroupMetadata] =
    for
  ...
  private[zarr] def parseArrayObject(
      root: JsonObject,
      attributes: JsonObject
  ): Either[ZarrError, V2ArrayMetadata] =
    for
      ...
      _ <- if chunks.length == shape.length then Right(())
      ...
      fill <- root.get("fill_value").toRight(ZarrError.InvalidMetadata(
        "$.zarray.fill_value",
      ...
      order <- requiredString(root, "order", "$.zarray.order").flatMap:
        ...
        case found => Left(ZarrError.InvalidMetadata("$.zarray.order", s"must be 'C' or 'F', found '$found'"))
  ...
  private[zarr] def parseAttributesObject(value: JsonValue, path: String): Either[ZarrError, JsonObject] =
    value match
      ...
      case Some(JsonValue.Arr(values)) =>
        ...
        while index < values.length do
          values(index) match
            ...
            case _ => return Left(ZarrError.InvalidMetadata(s"$$.zarray.filters[$index]", "must be an object"))
          index += 1
        Right(result.result())
      case Some(_) => Left(ZarrError.InvalidMetadata("$.zarray.filters", "must be an array or null"))
  ...
  private def dimensionSeparator(root: JsonObject): Either[ZarrError, ChunkSeparator] =
    root.get("dimension_separator") match
      ...
      case Some(_) => Left(ZarrError.InvalidMetadata("$.zarray.dimension_separator", "must be a string"))
  ...
  private def requiredLong(root: JsonObject, field: String, path: String): Either[ZarrError, Long] =
    root.get(field) match
      case Some(JsonValue.Num(number)) => number.toLongExact.left.map(ZarrError.InvalidMetadata(path, _))
      case Some(_) => Left(ZarrError.InvalidMetadata(path, "must be an integer"))
      case None => Left(ZarrError.InvalidMetadata(path, "missing required field"))

  private def requiredString(root: JsonObject, field: String, path: String): Either[ZarrError, String] =
    root.get(field) match
...
object V2ArrayDescriptor:
  ...
  def compile(
    ...
    for
      ...
      chunkGrid <- extension("regular", Vector(
        "chunk_shape" -> JsonValue.Arr(metadata.chunks.map(number))
      ))
      keyEncoding <- extension("v2", Vector(
  ...
  private def rejectFilters(filters: Vector[JsonObject]): Either[ZarrError, Unit] =
    filters.headOption match
      case None => Right(())
      case Some(filter) => Left(ZarrError.UnsupportedExtension(
        "v2 filter",
  ...
  private def codecs(
    ...
    if metadata.order == V2MemoryOrder.F then
      result += ExtensionMetadata(
        "transpose",
        JsonObject.unsafe(Vector(
          "order" -> JsonValue.Arr(metadata.shape.indices.reverse.map(index => number(index.toLong)).toVector)
        )),
        true,
        JsonObject.empty
      )
    ...
    metadata.compressor match
      ...
      case Some(compressor) => compressorExtension(compressor, dtype).map: extension =>
        result += extension
        result.result()
  ...
  private def compressorExtension(
    ...
    case Some("gzip") => Right(ExtensionMetadata(
      "gzip",
      compressor.removed(Set("id")),
      true,
      JsonObject.empty
    ))
    case Some("crc32c") => Right(ExtensionMetadata("crc32c", JsonObject.empty, true, JsonObject.empty))
  ...
  private def translateBlosc(
    ...
    val shuffle = compressor.get("shuffle") match
      case Some(JsonValue.Num(number)) => number.toLongExact.flatMap:
    ...
    shuffle.left.map(detail => ZarrError.InvalidMetadata("$.zarray.compressor.shuffle", detail)).map: found =>
  ...
  private def dimensionNames(
    ...
    case Some(JsonValue.Arr(values)) if values.length == rank && values.forall(_.isInstanceOf[JsonValue.Str]) =>
      Some(values.map:
  ...
  private def extension(
    ...
    JsonObject.from(fields).left.map(detail => ZarrError.InvalidMetadata("$.configuration", detail)).map: config =>
