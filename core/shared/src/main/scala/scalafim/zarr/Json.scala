...
final class JsonNumber private (val lexeme: String):
  def isInteger: Boolean = !lexeme.exists(character => character == '.' || character == 'e' || character == 'E')
  ...
  def toLongExact: Either[String, Long] =
    if !isInteger then Left(s"expected an integer, found $lexeme")
      ...
      catch case _: NumberFormatException => Left(s"integer is outside the signed 64-bit range: $lexeme")
...
object JsonNumber:
  private[zarr] def unsafe(lexeme: String): JsonNumber = new JsonNumber(lexeme)
...
object JsonObject:
  ...
  private[zarr] def unsafe(fields: Vector[(String, JsonValue)]): JsonObject =
...
object JsonParser:
  ...
  private final class Parser(input: String, limits: JsonLimits):
    ...
    private def number(): Either[JsonError, JsonNumber] =
      ...
      if offset >= input.length then return Left(JsonError.InvalidNumber(start, "missing integer digits"))
    ...
    private def string(): Either[JsonError, String] =
      ...
      while offset < input.length do
        ...
        if character == '"' then
          ...
          return Left(JsonError.UnexpectedCharacter(offset, character, "an escaped control character"))
    ...
    private def unicodeEscape(result: java.lang.StringBuilder): Either[JsonError, Unit] =
      ...
      hexCodeUnit() match
        ...
        case Right(high) if Character.isHighSurrogate(high) =>
          if offset + 1 >= input.length || input.charAt(offset) != '\\' || input.charAt(offset + 1) != 'u' then
            Left(JsonError.InvalidUnicode(escapeOffset, "escaped high surrogate lacks a low surrogate"))
          else
            offset += 2
    ...
    private def hexCodeUnit(): Either[JsonError, Char] =
      if offset + 4 > input.length then Left(JsonError.UnexpectedEnd(offset, "four hexadecimal digits"))
      else
        var value = 0
        var index = 0
        while index < 4 do
      ...
      if current('}') then
        ...
        while true do
          if count >= limits.maxObjectFields then
            return Left(JsonError.LimitExceeded(offset, "object field count", limits.maxObjectFields))
    ...
    private inline def isNonZeroDigit(character: Char): Boolean = character >= '1' && character <= '9'
...
object JsonRenderer:
  ...
  private def append(value: JsonValue, result: java.lang.StringBuilder): Unit = value match
    case JsonValue.Null => result.append("null")
    case JsonValue.Bool(found) => result.append(if found then "true" else "false")
    case JsonValue.Num(found) => result.append(found.lexeme)
    case JsonValue.Str(found) => appendString(found, result)
    case JsonValue.Arr(values) =>
      result.append('[')
      var index = 0
      while index < values.length do
        if index > 0 then result.append(',')
