...
object ShardIndexCodec:
  ...
  def encodedLength(
    ...
    for
      ...
      _ <- if entries <= limits.maxEntries.toLong then Right(())
      ...
      _ <- if total <= limits.maxIndexBytes.toLong then Right(())
  ...
  def decode(
    ...
    case Right(expected) =>
      ...
      else Crc32c.verifyAndStrip(encoded) match
...
final class ShardedGrid private (
  ...
  def locate(globalInnerCoordinate: ChunkCoordinate): Either[ZarrError, (ChunkCoordinate, ChunkCoordinate)] =
    if globalInnerCoordinate.rank.toInt != rank.toInt then
      Left(ZarrError.RankMismatch(rank.toInt, globalInnerCoordinate.rank.toInt, "inner chunk coordinate"))
    else
  ...
  def apply(
    ...
    if innerChunkShape.rank.toInt != rank then
      ...
      while axis < rank do
        ...
        if inner <= 0L then
          return Left(ZarrError.InvalidGrid(
...
object ShardPlanner:
  def group(
    ...
    while demandIndex < innerPlan.demands.length do
      ...
      demandIndex += 1
    val orderedKeys = groups.keys.toVector.sortWith((left, right) => compareVectors(left, right) < 0)
  ...
  def apply(
    ...
    ShardIndexCodec.encodedLength(innerGridShape, limits) match
      ...
      case Right(indexLength) =>
        ...
        ).flatMap(ByteCount.apply).map(total => new ShardIndexReadPlan(reads.result(), total))
...
final case class ShardDataPlan(shards: Vector[ShardDataRead], rangeReads: Int, plannedBytes: ByteCount)
  ...
  def resolve(
    ...
    while shardIndex < plan.shards.length do
      ...
      val index = indexes.get(shard.coordinate.toVector) match
        case None =>
          return Left(ZarrError.InvalidSelection(s"missing decoded index for shard ${shard.coordinate}"))
      ...
      while chunkIndex < shard.innerChunks.length do
        ...
        index.entry(inner.localCoordinate) match
          ...
          case Right(ShardIndexEntry.Present(offset, length)) =>
            ...
            rangeReads += 1
            chunks += ShardChunkRead(inner.localCoordinate, ShardChunkSource.Range(range), inner.copy)
        chunkIndex += 1
