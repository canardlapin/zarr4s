...
private[zarr] object WriteInternals:
  def arrayMetadata(
      descriptor: ArrayDescriptor,
      path: ZarrPath,
      limits: WriterLimits
  ): Either[ZarrError, (StoreKey, OwnedBytes)] =
    ZarrMetadataRenderer.array(descriptor).flatMap: rendered =>
  ...
  def groupMetadata(
    ...
    ZarrMetadataRenderer.group(group).flatMap: rendered =>
      metadata(rendered, path, limits)
  ...
  def assembleShard(
    ...
    else ShardIndexCodec.encodedLength(innerChunksPerShard, limits.shardIndex).flatMap: indexLength =>
      shardDataLength(chunks).flatMap: dataLength =>
        LongArrays.checkedAdd(dataLength, indexLength.toLong, "encoded shard bytes").flatMap: total =>
          ...
          if total > limit then
            ...
            ShardIndex(innerChunksPerShard, entries.result(), limits.shardIndex).flatMap: shardIndex =>
              ShardIndexCodec.encode(shardIndex, limits.shardIndex).map: encodedIndex =>
  ...
  private def shardDataLength(
    ...
    while index < chunks.length && failure.isEmpty do
      chunks(index) match
        case None => ()
        case Some(bytes) => LongArrays.checkedAdd(
            length,
            bytes.byteCount.toLong,
