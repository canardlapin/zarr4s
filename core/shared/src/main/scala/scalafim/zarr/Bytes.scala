...
object ByteCount:
  ...
  private[zarr] def unsafe(value: Long): ByteCount = value

  extension (count: ByteCount)
    inline def toLong: Long = count

final class OwnedBytes private (private[zarr] val values: Array[Byte]):
...
object OwnedBytes:
  ...
  private[zarr] def unsafe(values: Array[Byte]): OwnedBytes = new OwnedBytes(values)
...
object ByteRange:
  def apply(offset: Long, length: Long): Either[ZarrError, ByteRange] =
    if offset < 0L then Left(ZarrError.InvalidSelection(s"byte offset must be non-negative, found $offset"))
    else ByteCount(length).flatMap: count =>
      LongArrays.checkedAdd(offset, length, "byte range end").map(_ => new ByteRange(offset, count))
...
object Crc32c:
  ...
  private[zarr] def checksum(bytes: Array[Byte], offset: Int, length: Int): Long =
    var crc = 0xffffffff
    var index = offset
  ...
  def verifyAndStrip(bytes: OwnedBytes): Either[CodecError, OwnedBytes] =
    if bytes.length < 4 then Left(CodecError.CorruptData("crc32c", "payload is shorter than checksum"))
    else
  ...
  private[zarr] def putLittleEndianUInt32(bytes: Array[Byte], offset: Int, value: Long): Unit =
    var shift = 0
    while shift < 32 do
      bytes(offset + shift / 8) = ((value >>> shift) & 0xffL).toByte
      shift += 8

  private[zarr] def getLittleEndianUInt32(bytes: Array[Byte], offset: Int): Long =
    var result = 0L
    var index = 0
    while index < 4 do
