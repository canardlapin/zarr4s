...
object ZarrMetadataRenderer:
  def group(metadata: GroupMetadata): Either[ZarrError, String] =
    for
      known <- JsonObject.from(Vector(
        "zarr_format" -> number(3L),
        "node_type" -> JsonValue.Str("group"),
        "attributes" -> JsonValue.Obj(metadata.attributes)
      )).left.map(detail => ZarrError.InvalidMetadata("$", detail))
      merged <- JsonObject.from(known.fields ++ metadata.unknown.fields.filterNot: (name, _) =>
        known.contains(name)
      ).left.map(detail => ZarrError.InvalidMetadata("$", detail))
  ...
  def array(descriptor: ArrayDescriptor): Either[ZarrError, String] =
    for
      chunkGrid <- renderExtension("regular", Vector(
        "chunk_shape" -> longArray(descriptor.grid.chunkShape.toVector)
      ))
      chunkKey <- renderExtension(descriptor.chunkKeyEncoding.name, Vector(
      ...
      merged <- JsonObject.from(known.fields ++ descriptor.unknown.fields.filterNot: (name, _) =>
        known.contains(name)
      ).left.map(detail => ZarrError.InvalidMetadata("$", detail))
    yield JsonValue.Obj(merged).render

  private def layoutCodecs(layout: PhysicalLayout): Either[ZarrError, Vector[JsonValue]] = layout match
    ...
    case PhysicalLayout.Sharded(sharded, innerCodecs, indexCodecs, location, outerCodecs) =>
      for
        ...
        sharding <- renderExtension("sharding_indexed", Vector(
  ...
  private def renderExtension(
    ...
    JsonObject.from(configuration).left.map: detail =>
    ...
    .flatMap: config =>
      ...
      JsonObject.from(fields.result()).left.map: detail =>
  ...
  private def fillValue(fill: StoredScalar): JsonValue = fill match
    ...
    case StoredScalar.Floating(value) if value == Double.PositiveInfinity => JsonValue.Str("Infinity")
    case StoredScalar.Floating(value) if value == Double.NegativeInfinity => JsonValue.Str("-Infinity")
    case StoredScalar.Floating(value) => JsonValue.Num(JsonNumber.unsafe(java.lang.Double.toString(value)))
