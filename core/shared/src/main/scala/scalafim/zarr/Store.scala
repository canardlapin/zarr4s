...
/** Capability to create one immutable object.
  *
  * A successful call must make the complete object visible atomically. Existing
  * objects are never replaced: implementations return [[StoreError.AlreadyExists]].
  * Deletion, mutation, listing, and namespace transactions are deliberately not
...
final class MemoryStore private[zarr] (
...
) extends ObjectReader, ObjectWriter:
...
final class AsyncMemoryStore private (
    ...
    private val requests: mutable.ArrayBuffer[ObjectRequest],
    private val writes: mutable.ArrayBuffer[ObjectWrite]
) extends AsyncObjectReader, AsyncObjectWriter:
  ...
  def coalesce[A](
    ...
    while index < sorted.length do
      ...
      val range = ByteRange(start, end - start) match
        case Left(error) => return Left(error)
        case Right(found) => found
      val resolved = members.result().map: (memberRange, value) =>
        CoalescedMember(
          value,
