...
final class SyncCodecRuntime private (
  ...
  private[zarr] def decode(
    ...
    CodecRuntimeValidation.arrayTrace(program, decodedShape).flatMap: trace =>
      trace.encodedShape.elementCount.flatMap: elementCount =>
        expectedByteLength(dataType, elementCount).flatMap: expected =>
          if expected.toLong > limits.maxDecodedBytes.toLong then
            Left(ZarrError.CodecFailure(CodecError.DecodedLimitExceeded(
              limits.maxDecodedBytes.toLong,
              expected.toLong
            )))
          else decodeBytesAndArrays(encoded, program, dataType, elementCount, expected, limits, trace)
        ...
        program.stages(index) match
          ...
          case stage if stage.input == CodecRepresentation.Bytes &&
            ...
            executors.get(stage.name) match
              ...
              case Some(executor) => executor.decode(stage, bytes, expected, limits) match
                case Left(error) => return Left(ZarrError.CodecFailure(error))
                case Right(found) => bytes = found
          case _: ExecutableArrayCodec => ()
          case stage => return Left(ZarrError.UnsupportedRead(s"executable array codec ${stage.name}"))
        index -= 1
      byteCodec match
        case None => Left(ZarrError.InvalidCodecChain("missing bytes codec during decode"))
        case Some(codec) =>
          ScalarBytes.decode(bytes, dataType, codec.endianness, elementCount, limits)
            .left.map(ZarrError.CodecFailure.apply)
            .flatMap(block => CodecRuntimeValidation.decodeArrays(block, trace))

  private[zarr] def encode(
    ...
    if block.elementCount.toLong != expectedElements then
      return Left(ZarrError.InvalidSelection(
    ...
    while index < program.stages.length do
      program.stages(index) match
        ...
        case stage if stage.input == CodecRepresentation.Bytes &&
            stage.output == CodecRepresentation.Bytes => bytes match
          case None => return Left(ZarrError.InvalidCodecChain(
            s"codec ${stage.name} precedes the array-to-bytes stage"
          ))
          case Some(found) => executors.get(stage.name) match
            case None => return missing(stage.name)
            case Some(executor) => executor.encode(stage, found) match
              case Left(error) => return Left(ZarrError.CodecFailure(error))
              case Right(encoded) => bytes = Some(encoded)
        case stage => return Left(ZarrError.UnsupportedRead(
...
object SyncCodecRuntime:
  def apply(
    ...
    CodecRuntimeValidation.executorIndex(
      platform,
...
final class AsyncCodecRuntime private (
  ...
  private[zarr] def decode(
    ...
    CodecRuntimeValidation.arrayTrace(program, decodedShape) match
      ...
      case Right(trace) => trace.encodedShape.elementCount match
        case Left(error) => Future.successful(Left(error))
        case Right(elementCount) => expectedByteLength(dataType, elementCount) match
          case Left(error) => Future.successful(Left(error))
          case Right(expected) if expected.toLong > limits.maxDecodedBytes.toLong =>
            Future.successful(Left(ZarrError.CodecFailure(CodecError.DecodedLimitExceeded(
              limits.maxDecodedBytes.toLong,
              expected.toLong
            ))))
          case Right(expected) =>
            def stage(
              ...
              else program.stages(index) match
                case found: BytesCodec => Future.successful(Right(bytes -> Some(found)))
                case _: ExecutableArrayCodec => stage(index - 1, bytes)
                case codec if codec.input == CodecRepresentation.Bytes &&
                    codec.output == CodecRepresentation.Bytes => executors.get(codec.name) match
                  case None => Future.successful(missing(codec.name))
                  case Some(executor) => executor.decode(codec, bytes, expected, limits).flatMap:
                    case Left(error) => Future.successful(Left(ZarrError.CodecFailure(error)))
                    case Right(found) => stage(index - 1, found)
                case codec => Future.successful(Left(ZarrError.UnsupportedRead(
                  s"executable array codec ${codec.name}"
                )))

            stage(program.stages.length - 1, encoded).map:
              case Left(error) => Left(error)
              case Right((_, None)) => Left(ZarrError.InvalidCodecChain("missing bytes codec during decode"))
      ...
      case Right(_) =>
        def stage(
          ...
          if index >= program.stages.length then
            Future.successful(bytes.toRight(
              ZarrError.InvalidCodecChain("codec program did not produce bytes")
            ))
          else program.stages(index) match
            case codec: ExecutableArrayCodec =>
              if bytes.nonEmpty then Future.successful(Left(ZarrError.InvalidCodecChain(
                s"array codec ${codec.name} follows bytes"
              )))
              else codec.encodeArray(currentBlock, currentShape) match
                case Left(error) => Future.successful(Left(error))
            ...
            case codec: BytesCodec =>
              ...
              else ScalarBytes.encode(currentBlock, dataType, codec.endianness) match
                case Left(error) => Future.successful(Left(ZarrError.CodecFailure(error)))
                case Right(found) => checked(found, maxEncodedBytes).flatMap:
                  case Left(error) => Future.successful(Left(error))
                  case Right(value) => stage(index + 1, currentBlock, currentShape, Some(value))
            case codec if codec.input == CodecRepresentation.Bytes &&
                codec.output == CodecRepresentation.Bytes => bytes match
              case None => Future.successful(Left(ZarrError.InvalidCodecChain(
                s"codec ${codec.name} precedes the array-to-bytes stage"
              )))
              case Some(found) => executors.get(codec.name) match
                case None => Future.successful(missing(codec.name))
                case Some(executor) => executor.encode(codec, found).flatMap:
                  case Left(error) => Future.successful(Left(ZarrError.CodecFailure(error)))
                  case Right(encoded) => checked(encoded, maxEncodedBytes).flatMap:
                    case Left(error) => Future.successful(Left(error))
                    case Right(value) => stage(
                      index + 1,
                      currentBlock,
                      currentShape,
                      Some(value)
                    )
            case codec => Future.successful(Left(ZarrError.UnsupportedWrite(
              s"executable array codec ${codec.name}"
            )))
  ...
  private def checked(
    ...
    if bytes.byteCount.toLong > limit.toLong then Future.successful(Left(
      ...
      program.stages(index) match
        ...
        case stage if stage.input == CodecRepresentation.Bytes &&
          ...
          if !contains(stage.name) then
            return Left(ZarrError.CodecFailure(
