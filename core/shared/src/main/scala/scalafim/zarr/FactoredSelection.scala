...
final class AxisIndices private (private[zarr] val values: Array[Long]):
...
object AxisIndices:
  ...
  private[zarr] def unsafe(values: Array[Long]): AxisIndices =
...
object AxisSlice:
  def apply(
    ...
    if start < 0L then Left(ZarrError.InvalidSelection(s"slice start must be non-negative, found $start"))
    else if stopExclusive < 0L then Left(ZarrError.InvalidSelection(
      s"slice stop must be non-negative, found $stopExclusive"
    ))
    else if step <= 0L then Left(ZarrError.InvalidSelection(
      s"slice step must be positive, found $step; descending slices are not supported"
    ))
    else if stopExclusive < start then Left(ZarrError.InvalidSelection(
...
private[zarr] enum BoundAxisSelection:
...
final class FactoredSelection private (
    ...
    private[zarr] val axes: Vector[BoundAxisSelection],
...
private[zarr] final class LongOffsets private (private[zarr] val values: Array[Long]):
  def length: Int = values.length
  def apply(index: Int): Long = values(index)

private[zarr] object LongOffsets:
  def unsafe(values: Array[Long]): LongOffsets = new LongOffsets(LongArrays.copy(values))

private[zarr] enum IndexRun:
...
private[zarr] final case class AxisProjection(source: IndexRun, destination: IndexRun):
  require(source.length == destination.length, "source and destination axis projections must align")

  def length: Long = source.length

private[zarr] final case class FactoredChunkCopy(
