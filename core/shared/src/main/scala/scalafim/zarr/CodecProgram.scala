...
/** A validated codec chain.
  *
  * The constructor is private so a descriptor cannot contain an impossible
  * representation transition. Programs beginning with array values contain
  * exactly one array-to-bytes transition; programs beginning with bytes are
  * lawful byte identities or bytes-to-bytes chains.
  */
  ...
  def encodedArrayShape(decodedShape: Shape): Either[ZarrError, Shape] =
    if initial == CodecRepresentation.Bytes then Right(decodedShape)
      ...
      while index < stages.length do
        stages(index) match
          case codec: ExecutableArrayCodec => codec.encodedShape(shape) match
  ...
  def compile(
    ...
    while index < stages.length do
      ...
      if stage.input != representation then
        return Left(ZarrError.InvalidCodecChain(
    ...
    if representation != CodecRepresentation.Bytes then
      ...
      Left(ZarrError.InvalidCodecChain(
...
/** The executable Zarr v3 shard-index profile supported by the kernel.
  *
  * Keeping this as a value in `PhysicalLayout` makes the index pipeline
  * inspectable and leaves an honest extension point without pretending the
  * current uint64 index implementation supports arbitrary codecs.
  */
...
object ShardIndexProgram:
  private[zarr] def compile(stages: Vector[CompiledCodec]): Either[ZarrError, ShardIndexProgram] =
    stages match
