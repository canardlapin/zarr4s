...
final class OwnedBooleans private (private[zarr] val values: Array[Boolean]):
...
object OwnedBooleans:
  ...
  private[zarr] def unsafe(values: Array[Boolean]): OwnedBooleans = new OwnedBooleans(values)

final class OwnedShorts private (private[zarr] val values: Array[Short]):
  val length: Int = values.length
...
object OwnedShorts:
  ...
  private[zarr] def unsafe(values: Array[Short]): OwnedShorts = new OwnedShorts(values)

final class OwnedInts private (private[zarr] val values: Array[Int]):
  val length: Int = values.length
...
object OwnedInts:
  ...
  private[zarr] def unsafe(values: Array[Int]): OwnedInts = new OwnedInts(values)

final class OwnedLongs private (private[zarr] val values: Array[Long]):
  val length: Int = values.length
...
object OwnedLongs:
  ...
  private[zarr] def unsafe(values: Array[Long]): OwnedLongs = new OwnedLongs(values)

final class OwnedFloats private (private[zarr] val values: Array[Float]):
  val length: Int = values.length
...
object OwnedFloats:
  ...
  private[zarr] def unsafe(values: Array[Float]): OwnedFloats = new OwnedFloats(values)

final class OwnedDoubles private (private[zarr] val values: Array[Double]):
  val length: Int = values.length
...
object OwnedDoubles:
  ...
  private[zarr] def unsafe(values: Array[Double]): OwnedDoubles = new OwnedDoubles(values)

/** The primitive carrier used by an executable fixed-width data type.
  *
  * Signed and unsigned values deliberately share the same primitive arrays:
  * unsigned interpretation belongs to the data type, while the hot path keeps
  * compact JVM/Scala.js primitive storage and exact raw bits.
  */
enum ScalarKind(val byteWidth: Int):
  ...
  private[zarr] def allocate(
      fill: StoredScalar,
      size: Int,
      dataTypeName: String
  ): Either[ZarrError, PrimitiveBlock] = this match
    case Bool => ScalarFill.boolean(fill, dataTypeName).map: value =>
      PrimitiveBlock.Bool(OwnedBooleans.unsafe(Array.fill(size)(value)))
    case Signed8 => ScalarFill.signed(fill, dataTypeName).map: value =>
      PrimitiveBlock.Int8(OwnedBytes.unsafe(Array.fill(size)(value.toByte)))
    case Unsigned8 => ScalarFill.unsigned(fill, dataTypeName).map: value =>
      PrimitiveBlock.UInt8(OwnedBytes.unsafe(Array.fill(size)(value.toByte)))
    case Signed16 => ScalarFill.signed(fill, dataTypeName).map: value =>
      PrimitiveBlock.Int16(OwnedShorts.unsafe(Array.fill(size)(value.toShort)))
    case Unsigned16 => ScalarFill.unsigned(fill, dataTypeName).map: value =>
      PrimitiveBlock.UInt16(OwnedShorts.unsafe(Array.fill(size)(value.toShort)))
    case Signed32 => ScalarFill.signed(fill, dataTypeName).map: value =>
      PrimitiveBlock.Int32(OwnedInts.unsafe(Array.fill(size)(value.toInt)))
    case Unsigned32 => ScalarFill.unsigned(fill, dataTypeName).map: value =>
      PrimitiveBlock.UInt32(OwnedInts.unsafe(Array.fill(size)(value.toInt)))
    case Signed64 => ScalarFill.signed(fill, dataTypeName).map: value =>
      PrimitiveBlock.Int64(OwnedLongs.unsafe(Array.fill(size)(value)))
    case Unsigned64 => ScalarFill.unsigned(fill, dataTypeName).map: value =>
      PrimitiveBlock.UInt64(OwnedLongs.unsafe(Array.fill(size)(value)))
    case Float32 => ScalarFill.floating(fill, 8).map: bits =>
      PrimitiveBlock.Float32(OwnedFloats.unsafe(Array.fill(size)(java.lang.Float.intBitsToFloat(bits.toInt))))
    case Float64 => ScalarFill.floating(fill, 16).map: bits =>
      PrimitiveBlock.Float64(OwnedDoubles.unsafe(Array.fill(size)(java.lang.Double.longBitsToDouble(bits))))
...
enum PrimitiveBlock:
  ...
  private[zarr] def copyElementFrom(
  ...
  private[zarr] def reordered(sourceIndices: Array[Int]): PrimitiveBlock = this match
...
private object ScalarFill:
  ...
  def signed(fill: StoredScalar, name: String): Either[ZarrError, Long] = fill match
    ...
    case _ => Left(ZarrError.InvalidMetadata("$.fill_value", s"$name requires a signed integral fill"))
  ...
  def unsigned(fill: StoredScalar, name: String): Either[ZarrError, Long] = fill match
    ...
    case _ => Left(ZarrError.InvalidMetadata("$.fill_value", s"$name requires an unsigned integral fill"))
  ...
  def floating(fill: StoredScalar, hexDigits: Int): Either[ZarrError, Long] = fill match
    ...
    case _ => Left(ZarrError.InvalidMetadata("$.fill_value", "floating dtype requires floating fill"))
...
object ScalarBytes:
  def decode(
    ...
    expectedBytes(dataType, elementCount, limits).flatMap: expected =>
      DecodedLength.validate(encoded, expected, limits).flatMap: bytes =>
        if elementCount > Int.MaxValue.toLong then
          ...
          dataType.scalarKind match
            ...
            case ScalarKind.Unsigned8 => Right(PrimitiveBlock.UInt8(OwnedBytes.copyOf(bytes.values)))
            case ScalarKind.Signed16 => Right(PrimitiveBlock.Int16(decodeShorts(bytes.values, count, order)))
            case ScalarKind.Unsigned16 => Right(PrimitiveBlock.UInt16(decodeShorts(bytes.values, count, order)))
            case ScalarKind.Signed32 => Right(PrimitiveBlock.Int32(decodeInts(bytes.values, count, order)))
            case ScalarKind.Unsigned32 => Right(PrimitiveBlock.UInt32(decodeInts(bytes.values, count, order)))
            case ScalarKind.Signed64 => Right(PrimitiveBlock.Int64(decodeLongs(bytes.values, count, order)))
            case ScalarKind.Unsigned64 => Right(PrimitiveBlock.UInt64(decodeLongs(bytes.values, count, order)))
            case ScalarKind.Float32 => Right(PrimitiveBlock.Float32(decodeFloats(bytes.values, count, order)))
            case ScalarKind.Float64 => Right(PrimitiveBlock.Float64(decodeDoubles(bytes.values, count, order)))
  ...
  private def decodeBooleans(bytes: Array[Byte]): Either[CodecError, OwnedBooleans] =
    ...
    while index < bytes.length do
      bytes(index) match
        ...
        case found => return Left(CodecError.CorruptData("bytes", s"invalid bool byte ${found & 0xff} at element $index"))
      index += 1
