...
final case class ExecutionReceipt private[zarr] (
    objectRequests: Int,
    rangeRequests: Int,
    lengthRequests: Int,
    bytesRead: Long,
...
final class OpenedArray private[zarr] (
    store: ObjectReader,
    val path: ZarrPath,
    ...
    if selection.arrayShape != descriptor.shape then
      ...
      PrimitiveBlockBuilder(
        ...
        val folded = foldFragments(selection, builder, limits): (current, fragment) =>
          PrimitiveBlockBuilder.applyFragment(
            current,
            fragment,
            selection.outputShape
  ...
  def foldFragments[S](
    ...
    case PhysicalLayout.Sharded(sharded, innerCodecs, _, location, outerCodecs) =>
      if outerCodecs.nonEmpty then
        Left(ZarrError.UnsupportedRead("outer codecs after sharding_indexed prevent bounded partial reads"))
  ...
  private def foldDirectFragments[S](
    ...
    while demandIndex < plan.demands.length && !stopped do
      ...
      val copy = demand.copy match
        ...
        case _ => return Left(ZarrError.InvalidSelection("fragment plan contains a non-factored copy"))
  ...
  private def foldShardedFragments[S](
    ...
    def emit(
      ...
      val fragment = decoded match
        ...
        case Some((block, shape)) => ChunkFragment.decoded(
          coordinate,
          block,
          shape,
          copy,
    ...
    while shardIndex < grouped.shards.length && !stopped do
      ...
      while innerIndex < shard.innerChunks.length && !stopped do
        ...
        if missingShard then
          ...
          case Left(error) => return Left(error)
          case Right(ShardIndexEntry.Fill) => emit(global, copy, None) match
            case Left(error) => return Left(error)
            case Right(_) => ()
  ...
  private def read(
    ...
    PrimitiveBlockBuilder(descriptor.dataType, descriptor.fillValue, outputShape).flatMap: builder =>
  ...
  private def executeShards(
    ...
    if grouped.touchedShards > limits.maxObjects then
      ...
      val presentShards = grouped.shards.filterNot(shard => missing.contains(shard.coordinate.toVector))
      val presentPlan = ShardReadPlan(presentShards, presentShards.map(_.innerChunks.length).sum, presentShards.length)
      val indexPlan = ShardIndexReadPlan(
        presentPlan,
        sharded.innerChunksPerShard,
        location,
      ...
      grouped.shards.foreach: shard =>
        if missing.contains(shard.coordinate.toVector) then indexes.update(shard.coordinate.toVector, fillIndex)
          ...
          store.read(key, read.range) match
            ...
            case Right(bytes) =>
              ...
              while memberIndex < read.members.length do
                ...
                val encoded = bytes.slice(member.relativeOffset, member.relativeOffset + member.length)
...
final class OpenedGroup private[zarr] (
    store: ObjectReader,
    val path: ZarrPath,
  ...
  def open(relativePath: String): Either[ZarrError, OpenedNode] =
    path.resolve(relativePath).flatMap: resolved =>
      index.flatMap(_.document(resolved)) match
        case Some(document) => SyncZarr.openDocument(
          store,
          resolved,
          document,
          index,
...
object SyncZarr:
  ...
  def openNode(
    ...
    readOptional(store, path, "zarr.json", limits.maxMetadataBytes).flatMap:
      case Some(json) => ZarrMetadata.parse(json).flatMap:
        case ZarrNodeMetadata.Group(group) =>
          ...
          indexed.map: found =>
            OpenedNode.Group(new OpenedGroup(
              store,
              path,
              group,
              ZarrFormat.V3,
        ...
        case ZarrNodeMetadata.Array(array) => openDocument(
          store,
          path,
    ...
    case NodeKind.Array => document.arrayDescriptor(capabilities).flatMap: descriptor =>
      OpenValidation.descriptor(descriptor, limits).flatMap: _ =>
        OpenValidation.codecPrograms(descriptor, runtime.validate).map: _ =>
          OpenedNode.Array(new OpenedArray(store, path, descriptor, document.format, runtime))
  ...
  private def openV2(
    ...
    val indexed = consolidation match
      case ConsolidationMode.Ignore => Right(None)
      case _ => readOptional(store, ZarrPath.root, ".zmetadata", limits.maxMetadataBytes).flatMap:
        case Some(json) => HierarchyIndex.v2(ZarrPath.root, json, limits.hierarchy).map(Some.apply)
        case None if consolidation == ConsolidationMode.Require => Left(ZarrError.InvalidMetadata(
          "$.zmetadata",
          "required consolidated metadata is absent"
        ))
    ...
    indexed.flatMap: found =>
      found.flatMap(_.document(path)) match
        case Some(document) => openDocument(
          store,
          path,
          document,
          found,
  ...
  private def openV2Individual(
    ...
    readOptional(store, path, ".zarray", limits.maxMetadataBytes).flatMap:
      case Some(arrayJson) => readOptional(store, path, ".zattrs", limits.maxMetadataBytes).flatMap: attributes =>
        V2Metadata.parseArray(arrayJson, attributes).flatMap: array =>
          openDocument(
            store,
            path,
      ...
      case None => readOptional(store, path, ".zgroup", limits.maxMetadataBytes).flatMap:
        case Some(groupJson) => readOptional(store, path, ".zattrs", limits.maxMetadataBytes).flatMap: attributes =>
          V2Metadata.parseGroup(groupJson, attributes).map: group =>
            OpenedNode.Group(new OpenedGroup(
              store,
              path,
              group,
        ...
        case None => path.key(".zarray").flatMap: key =>
  ...
  private def readOptional(
      ...
      child: String,
      limit: ByteCount
  ): Either[ZarrError, Option[String]] = path.key(child).flatMap: key =>
