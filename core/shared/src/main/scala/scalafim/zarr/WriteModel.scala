...
trait ChunkProvider:
  /** Returns the stored chunk at a global inner-chunk coordinate.
    *
    * For a direct layout this is the stored chunk coordinate. For a sharded
    * layout it is the global coordinate in the inner-chunk grid. `storedShape`
    * is always the grid's full nominal chunk shape, including at array borders;
    * the provider must fill any overhang with the array's fill value.
    */
  def chunk(
...
final class WriteProgress private[zarr] (
...
/** A complete array or group write. Metadata is created last and is therefore
  * the completion marker for store-independent publication.
  */
final class WriteReceipt private[zarr] (
...
private[zarr] final class WriteMetrics:
  ...
  def visit(limits: WriterLimits): Either[ZarrError, Unit] =
    LongArrays.checkedAdd(visited, 1L, "visited chunks").flatMap: next =>
  ...
  def encodedChunk(): Either[ZarrError, Unit] =
    LongArrays.checkedAdd(encoded, 1L, "encoded chunks").map: next =>
      encoded = next

  def omitFill(): Either[ZarrError, Unit] =
    LongArrays.checkedAdd(omitted, 1L, "omitted fill chunks").map: next =>
      omitted = next

  def omitPadding(): Either[ZarrError, Unit] =
    LongArrays.checkedAdd(padding, 1L, "shard padding chunks").map: next =>
      padding = next
  ...
  def permitDataObject(
    ...
    if requestedObjects > limits.maxObjects.toLong then
      ...
      _ <- if total <= limits.maxWrittenBytes.toLong then Right(())
  ...
  def record(objectValue: WrittenObject): Either[ZarrError, Unit] =
    LongArrays.checkedAdd(bytes, objectValue.length.toLong, "written data bytes").map: next =>
      written += objectValue
      bytes = next
...
private[zarr] object PrimitiveBlockType:
  def validate(
