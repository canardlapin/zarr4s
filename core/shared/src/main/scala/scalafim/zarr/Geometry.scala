...
private[zarr] object LongArrays:
  ...
  def checkedAdd(left: Long, right: Long, context: String): Either[ZarrError, Long] =
    if right > 0L && left > Long.MaxValue - right then
...
object Rank:
  ...
  private[zarr] def unsafe(value: Int): Rank = value

  extension (rank: Rank)
    inline def toInt: Int = rank

final class Shape private (private[zarr] val values: Array[Long]):
  ...
  def from(dimensions: Seq[Long]): Either[ZarrError, Shape] =
    ...
    while axis < values.length do
      if values(axis) < 0L then
        return Left(ZarrError.InvalidShape(
          s"dimension $axis must be non-negative, found ${values(axis)}"
        ))
      axis += 1
    Right(unsafe(values))

  private[zarr] def unsafe(dimensions: Array[Long]): Shape =
    new Shape(LongArrays.copy(dimensions))

final class Coordinate private (private[zarr] val values: Array[Long]):
  val rank: Rank = Rank.unsafe(values.length)
...
object Coordinate:
  ...
  def from(indices: Seq[Long]): Either[ZarrError, Coordinate] =
    ...
    while axis < values.length do
      if values(axis) < 0L then
        return Left(ZarrError.InvalidCoordinate(
          s"index $axis must be non-negative, found ${values(axis)}"
        ))
      axis += 1
    Right(unsafe(values))

  private[zarr] def unsafe(indices: Array[Long]): Coordinate =
...
final class CoordinateBatch private (
    val shape: Shape,
    val count: Int,
    private[zarr] val flattened: Array[Long]
):
...
object CoordinateBatch:
  def within(shape: Shape, coordinates: Seq[Coordinate]): Either[ZarrError, CoordinateBatch] =
    ...
    if rank != 0 && coordinates.size > Int.MaxValue / rank then
      Left(ZarrError.ResourceLimit("coordinate storage", Int.MaxValue, coordinates.size.toLong * rank))
    else
