...
object BuiltInDataTypes:
  ...
  val int8: DataTypeCapability = SignedIntegerDataType("int8", ScalarKind.Signed8, Byte.MinValue, Byte.MaxValue)
  val int16: DataTypeCapability = SignedIntegerDataType("int16", ScalarKind.Signed16, Short.MinValue, Short.MaxValue)
  val int32: DataTypeCapability = SignedIntegerDataType("int32", ScalarKind.Signed32, Int.MinValue, Int.MaxValue)
  val int64: DataTypeCapability = SignedIntegerDataType("int64", ScalarKind.Signed64, Long.MinValue, Long.MaxValue)
...
final case class BytesCodec(endianness: Option[Endianness]) extends CompiledCodec:
  ...
  val configuration: JsonObject = endianness match
    ...
    case Some(found) => JsonObject.unsafe(Vector(
...
object BuiltInCodecs:
          ...
          values(index) match
            case JsonValue.Num(number) => number.toLongExact match
              case Right(found) if found >= Int.MinValue.toLong && found <= Int.MaxValue.toLong =>
                result += found.toInt
              case Right(found) => return Left(s"$field[$index] is outside the 32-bit integer range: $found")
  ...
  private def optionalString(objectValue: JsonObject, field: String): Either[String, Option[String]] =
    objectValue.get(field) match
      case None => Right(None)
...
object ZarrMetadata:
  ...
  private val arrayFields = Set(
    "zarr_format", "node_type", "shape", "data_type", "chunk_grid",
  ...
  def parse(input: String, limits: JsonLimits = JsonLimits()): Either[ZarrError, ZarrNodeMetadata] =
    JsonParser.parse(input, limits).left.map(ZarrError.InvalidJson.apply).flatMap:
  ...
  private[zarr] def parseExtension(
      value: JsonValue,
      path: String
  ): Either[ZarrError, ExtensionMetadata] = value match
    case JsonValue.Str(name) => Right(ExtensionMetadata(name, JsonObject.empty, true, JsonObject.empty))
  ...
  private[zarr] def parseExtensions(
  ...
  private def requiredLong(
    ...
    case JsonValue.Num(number) => number.toLongExact.left.map(detail => ZarrError.InvalidMetadata(path, detail))
...
object ArrayDescriptor:
  def compile(
    ...
    for
      ...
      fill <- dataType.parseFill(metadata.fillValue).left.map: detail =>
  ...
  private def compileDataType(
    ...
    else capabilities.dataType(extension.name).toRight(ZarrError.UnsupportedDataType(extension.name))
    ...
    if !extension.mustUnderstand then
      ...
      extension.configuration.get("chunk_shape") match
          ...
          while index < values.length do
            values(index) match
              case JsonValue.Num(number) => number.toLongExact match
                ...
                case Left(detail) =>
                  return Left(ZarrError.InvalidMetadata(s"$$.chunk_grid.configuration.chunk_shape[$index]", detail))
              case _ =>
                return Left(ZarrError.InvalidMetadata(
        ...
        case Some(_) => Left(ZarrError.InvalidMetadata(
          "$.chunk_grid.configuration.chunk_shape", "must be an array"
        ))
        case None => Left(ZarrError.InvalidMetadata(
          "$.chunk_grid.configuration.chunk_shape", "missing required field"
        ))
  ...
  private def compileChunkKey(
    ...
    if !extension.mustUnderstand then
      Left(ZarrError.InvalidMetadata("$.chunk_key_encoding.must_understand", "false is not permitted"))
    else extension.name match
      case "default" => compileSeparator(extension, ChunkSeparator.Slash).map(DefaultChunkKeyEncoding.apply)
  ...
  private def compileSeparator(
    ...
    case Some(JsonValue.Str(found)) => Left(ZarrError.InvalidMetadata(
      "$.chunk_key_encoding.configuration.separator",
      s"must be '/' or '.', found '$found'"
    ))
    case Some(_) => Left(ZarrError.InvalidMetadata(
      "$.chunk_key_encoding.configuration.separator", "must be a string"
    ))
  ...
  private def compileStorageTransformers(
    ...
    extensions.find(_.mustUnderstand) match
      case Some(extension) => Left(ZarrError.UnsupportedExtension("storage transformer", extension.name))
      case None => Right(())
  ...
  private def compileIndexCodecs(
    ...
    case Vector(bytes, crc) if bytes.name == "bytes" && crc.name == "crc32c" =>
      BuiltInCodecs.bytes.compile(bytes, IndexUInt64DataType) match
        case Left(detail) => Left(ZarrError.InvalidMetadata(
        ...
        case Right(_) => Left(ZarrError.InvalidMetadata(
          "$.codecs[0].configuration.index_codecs[0]",
          "first supported index codec must be little-endian bytes"
        ))
    case _ => Left(ZarrError.UnsupportedExtension(
