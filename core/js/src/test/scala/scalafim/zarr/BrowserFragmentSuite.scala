...
class BrowserFragmentSuite extends munit.FunSuite:
  ...
  test("browser fold emits compact factored fragments"):
    ...
    BrowserZarr.openArray(store).flatMap:
      case Left(error) => fail(error.message)
      case Right(opened) =>
        val selected = zvalue(FactoredSelection.within(opened.descriptor.shape, Vector(
  ...
  test("browser sharded fold preserves bounded backpressure and early stop"):
    ...
    BrowserZarr.openArray(store).flatMap:
      ...
      case Right(opened) =>
        ...
        store.clearTrace()
        val folded = opened.foldFragments(selected, 0): (count, _) =>
          entered.trySuccess(())
          release.future
  ...
  test("browser fragment fold propagates decode failure before callback"):
    ...
    BrowserZarr.openArray(store).flatMap:
      ...
      case Right(opened) =>
        var calls = 0
        opened.foldFragments(FactoredSelection.all(opened.descriptor.shape), ()): (_, _) =>
          calls += 1
