            ...
            opened
              ...
              .flatMap:
                ...
                case Right(result) =>
                  ...
                  assertEquals(
                    firstTrace.collect { case request @ ObjectRequest.Range(_, _) =>
                      request
                    }.length,
                    2
                  )
      ...
      objects.get(path) match
        ...
        case Some(payload) =>
          ...
          if method == "HEAD" then
            ...
            val _ = response.end(uint8(selected))
          else
            response.statusCode = 200
            response.setHeader("Content-Length", all.length.toString)
            val _ = response.end(uint8(all))
