...
object BrowserBloscZstd extends AsyncByteCodecExecutor:
  ...
  def decode(
    ...
    case found: BloscZstdCodec =>
      BloscFrame.validate(found, encoded, expectedDecoded, limits) match
        ...
        case Right(_) => transform(found, encoded, encode = false).map:
  ...
  /**
   * Encode using numcodecs.js. Its Blosc binding fixes `typesize` at four and
   * does not expose it, so accepting another stride here would silently write
   * metadata-inconsistent shuffle frames.
   */
  def encode(
      codec: CompiledCodec,
      decoded: OwnedBytes
    ...
    case found: BloscZstdCodec if found.typeSize.toInt != 4 =>
      Future.successful(Left(CodecError.UnsupportedCapability(
        s"blosc encode with typesize ${found.typeSize.toInt}",
        "Scala.js numcodecs"
      )))
    case found: BloscZstdCodec => transform(found, decoded, encode = true).map:
      case Left(error) => Left(error)
      case Right(encoded) =>
        BloscFrame.validate(
          found,
  ...
  private def transform(
    ...
    try
      val algorithm = NumcodecsBlosc.fromConfig(js.Dynamic.literal(
        id = "blosc",
        cname = "zstd",
      ...
      promise.toFuture.map(output => Right(fromUint8Array(output))).recover:
        case NonFatal(error) => Left(corrupt(error))
    catch
      case NonFatal(error) => Future.successful(Left(corrupt(error)))
...
object BrowserBloscZstdRuntime:
  val portable: AsyncCodecRuntime = AsyncCodecRuntime(
    ...
    Vector(BrowserGzip, BrowserBloscZstd)
  ) match
