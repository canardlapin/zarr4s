...
class BrowserBloscZstdSuite extends munit.FunSuite:
  ...
  test("Scala.js provider round-trips the observed typesize-four surface"):
    Future.sequence(BloscShuffle.values.toVector.map: shuffle =>
      ...
      BrowserBloscZstd.encode(codec, decoded).flatMap:
        case Left(error) => fail(error.message)
        case Right(encoded) => BrowserBloscZstd.decode(
          codec,
          encoded,
          decodedCount,
  ...
  test("Scala.js provider refuses metadata-inconsistent non-four-byte writes"):
    ...
    BrowserBloscZstd.encode(codec, decoded).map: result =>
    ...
    BrowserBloscZstd.encode(codec, decoded).flatMap:
      ...
      case Right(encoded) =>
        ...
        BrowserBloscZstd.decode(
          ...
          assertEquals(
            result,
            Left(CodecError.InvalidDecodedLength(
